# -*- coding: utf-8 -*-
"""
Created on Sun Feb 22 08:59:17 2026
...
# -*- coding: utf-8 -*-
"""
Phase 4 v3.0: TRUE DATA-PARALLEL OOT INFERENCE ENGINE
- [SYNC] Fully synchronized with Phase 3 v3.0 (K=5 Summary Fusion, Data Parallelism).
- [UPDATE] Loads 7 weights (1 Intercept + 6 Fusion Features) correctly.
- [UPDATE] Batches are split 50/50 across GPUs for true 100% Dual-GPU utilization.
- [UPDATE] Independent 2D/3D Top-K extraction and logit calculation.
"""
...
class STPUltraFastRecommender:
    ...
    def _load_brain(self):
        ...
        print(f"   - Loaded weights for Fusion Features (K={self.K}).")
    ...
    def _initialize_gpu_engine(self):
        ...
        # [핵심 수정] FULL_N 사이즈에 맞춘 완벽한 스캐폴드 마스킹
        ...
        if 'publication_year' in full_df.columns:
            ...
            self.future_df = full_df[full_df['publication_year'] == 2024].copy()
            past_df = full_df[full_df['publication_year'] <= self.CUTOFF_YEAR].copy()
            print(f"   - Temporal Split: References(<= {self.CUTOFF_YEAR}) vs Test(== 2024)")
        else:
    ...
    def _run_gpu_half_inference(self, q_idx, q_scaf, q_ha, device_id):
        """배치의 절반을 전담하여 3000여 개 타겟에 대한 확률(Probability) 행렬 반환"""
        B = len(q_idx)
        if B == 0: return None
        ...
        local_weights = self.weights[ha_bins].to(device) # (B, 7)
        ...
        for t_idx, actives in enumerate(actives_list):
            ...
            # --- Top-K 요약 통계량 추출 (Phase 3와 동일 로직) ---
            k_act = min(self.K, M)
    ...
    def evaluate_oot_top_n(self):
        if self.future_df.empty:
            print(">> No future data (2024) found for OOT evaluation.")
            return
            
        print(f"\n>> 3. Evaluating Top-N Hit Rate on 2024 OOT Data (Data-Parallel Engine)...")
        ...
        with tqdm(total=total_queries, desc="Dual-GPU Screening") as pbar:
            for i in range(0, total_queries, self.BATCH_SIZE):
                # Data Parallel: 배치를 반으로 쪼개서 두 GPU에 비동기 전달
        ...
        if csv_results:
            out_file = "features_store/stp_OOT_predictions_Top15_FusionK5.csv"
...
if __name__ == "__main__":
    recommender = STPUltraFastRecommender(
        ...
        coef_json="features_store/stp_coef_FusionK5_paired_sum.json", # [수정됨] K=5 최신 뇌 파일 장착
